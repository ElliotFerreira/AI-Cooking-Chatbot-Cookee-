      ...
      const response = await fetch('http://127.0.0.1:8000/api/generate-conversation-title', {
        ...
        body: JSON.stringify({
          messages : [
            {
              role: "system",
              content: "You are a friendly cooking assistant named Cookee. Based on the current conversation give the conversation a name, make sure it's less than 5 words. Of absolute importance make sure the response does not come back with any quotation marks if the request is empty please throw an error."
            },
